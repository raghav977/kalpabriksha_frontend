...
export default function EditEventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: event, isLoading, error } = useEvent(parseInt(id));
  ...
  useEffect(() => {
    if (event) {
      ...
      const eventDate = (event as any).date
        ? new Date((event as any).date).toISOString().split('T')[0]
        : '';
  ...
  const updateMutation = useUpdateEvent(parseInt(id));
  ...
  if (isLoading) {
    return (
      <div>
        <PageHeader title="Edit Event" backButton />
        <LoadingSpinner />
      </div>
    );
  }
    ...
    return (
      <div>
        ...
        <ErrorState
          ...
          onRetry={() => router.push('/admin/events')}
        />
      </div>
    );
  }
